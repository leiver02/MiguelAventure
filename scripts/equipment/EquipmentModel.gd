...
func _ready():
	...
	# Si no configuraste ranuras en el Inspector, frenamos el código aquí para que no falle
	if slots.is_empty():
		push_warning("EquipmentModel: No se configuraron slots de equipamiento en el Inspector.")
		return

	for slot_type in slots.keys():
		var slot = slots[slot_type]
		# Añadimos seguridad extra por si acaso una de las ranuras dentro del diccionario está vacía
		if slot:
